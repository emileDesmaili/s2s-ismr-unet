...
def main():
    ...
    x, y = dataloader.get_data(years=years, download = download,
                            week=week,obs=obs,custom_lead = (17,30),
                                domain=domain, season=season, 
                            model=model)
    
    # for ECMF we pad the data with zeros to make it divisible by 8 (no other way around)
    #pad the data to make it 24x24 from 23x24
    x = x.pad(Y=(0,1))
    #replace nan Y value with 40.5
    x['Y'] = x['Y'].fillna(40.5)
    x = x.fillna(0)
    y = y.pad(Y=(0,1))
    y['Y'] = y['Y'].fillna(40.5)
