...
def main():
    ...
    dir = "2MME/"
    obs = "IMD"
    models = ["IITM", "ECMWF"] # can be GEFS IITM ECMWF for multi-model training, put a list of strings
    ...
    print(f' #######  TUNING 2-MME for {obs} for {week} lead time  ##########')
    ...
    custom_leads = {'IITM':(16,29), 'ECMWF':(16,30)} # custom lead times for each model
    custom_seasons = {'IITM':"May-Sep", 'ECMWF':"May-Sep"} # custom season for each model
    ...
    rpss_train_elr.to_netcdf('outputs/' + (dir or '') + f'2MME_{obs}/ELR_rpss_train_{week}.nc')
    rpss_test_elr.to_netcdf('outputs/' + (dir or '') + f'2MME_{obs}/ELR_rpss_test_{week}.nc')
    ...
    for i in range(n_bootstraps):
        print('Bootstrap', i+1)
        print('Train years:', set(xtrain_dict['ECMWF'][i]['T'].dt.year.values))
        print('Validation years:', set(xval_dict['ECMWF'][i]['T'].dt.year.values))
        print('Test years:', set(xtest_dict['ECMWF'][i]['T'].dt.year.values))
        print('')
